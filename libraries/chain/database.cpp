...
#include <CreateCoin/chain/compound.hpp>
...
#include <CreateCoin/chain/db_with.hpp>
...
#include <CreateCoin/chain/CreateCoin_evaluator.hpp>
#include <CreateCoin/chain/CreateCoin_objects.hpp>
#include <CreateCoin/chain/transaction_object.hpp>
#include <CreateCoin/chain/shared_db_merkle.hpp>
...
#include <CreateCoin/chain/util/uint256.hpp>
#include <CreateCoin/chain/util/reward.hpp>
#include <CreateCoin/chain/util/manabar.hpp>
#include <CreateCoin/chain/util/rd_setup.hpp>
...
FC_REFLECT( CreateCoin::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( CreateCoin::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( CreateCoin::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
struct reward_fund_context
{
   ...
   share_type  CreateCoin_awarded = 0;
};
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&note) {
      CreateCoin_TRY_NOTIFY(this_->_post_reindex_signal, note);
   } BOOST_SCOPE_EXIT_END

   try
   {
      CreateCoin_TRY_NOTIFY(_pre_reindex_signal, note);
      ...
      CreateCoin_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return CreateCoin_chain_id;
}

void database::set_chain_id( const chain_id_type& chain_id )
{
   CreateCoin_chain_id = chain_id;

   idump( (CreateCoin_chain_id) );
}
...
{ try {
   if( !has_hardfork( CreateCoin_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( CreateCoin_HARDFORK_0_17__769 ) || comment.parent_author == CreateCoin_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(CreateCoin_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
...
void database::pop_block()
{
   try
   {
      ...
      CreateCoin_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
{
   CreateCoin_TRY_NOTIFY( _pre_apply_operation_signal, note )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = CreateCoin_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / CreateCoin_BLOCK_INTERVAL + 1;
}

/**
 *  Converts CreateCoin into sbd and adds it to to_account while reducing the CreateCoin supply
 *  by CreateCoin and increasing the sbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_sbd( const account_object& to_account, asset CreateCoin, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, SBD_SYMBOL ), asset( 0, CreateCoin_SYMBOL ) );

   try
   {
      if( CreateCoin.amount == 0 )
         return assets;
      ...
      {
         auto to_sbd = ( gpo.sbd_print_rate * CreateCoin.amount ) / CreateCoin_100_PERCENT;
         auto to_CreateCoin = CreateCoin.amount - to_sbd;

         auto sbd = asset( to_sbd, CreateCoin_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, sbd );
            adjust_reward_balance( to_account, asset( to_CreateCoin, CreateCoin_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, sbd );
            adjust_balance( to_account, asset( to_CreateCoin, CreateCoin_SYMBOL ) );
         }

         adjust_supply( asset( -to_sbd, CreateCoin_SYMBOL ) );
         adjust_supply( sbd );
         assets.first = sbd;
         assets.second = asset( to_CreateCoin, CreateCoin_SYMBOL );
      }
      else
      {
         adjust_balance( to_account, CreateCoin );
         assets.second = CreateCoin;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(CreateCoin) )
...
asset create_vesting2( database& db, const account_object& to_account, asset liquid, bool to_reward_balance, Before&& before_vesting_callback )
{
   try
   {
      auto calculate_new_vesting = [ liquid ] ( price vesting_share_price ) -> asset
         {
         /**
          *  The ratio of total_vesting_shares / total_vesting_fund_CreateCoin should not
      ...
      FC_ASSERT( liquid.symbol == CreateCoin_SYMBOL );
      ...
      {
         if( db.has_hardfork( CreateCoin_HARDFORK_0_20__2539 ) )
         {
            db.modify( to_account, [&]( account_object& a )
            {
               util::manabar_params params( util::get_effective_vesting_shares( a ), CreateCoin_VOTING_MANA_REGENERATION_SECONDS );
FC_TODO( "Set skip_cap_regen=true without breaking consensus" );
      ...
      db.modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_CreateCoin += liquid;
         }
         else
         {
            props.total_vesting_fund_CreateCoin += liquid;
 ...
 * @param liquid     - CreateCoin or liquid SMT to be converted to vesting shares
 */
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( CreateCoin_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   ...
                                   int depth )
{
   if( a.proxy != CreateCoin_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= CreateCoin_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = CreateCoin_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = CreateCoin_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( CreateCoin_HARDFORK_0_2 ) )
         w.virtual_scheduled_time = w.virtual_last_update + (CreateCoin_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
      else
         w.virtual_scheduled_time = w.virtual_last_update + (CreateCoin_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

      /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
      if( has_hardfork( CreateCoin_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( CreateCoin_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( CreateCoin_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( CreateCoin_NULL_ACCOUNT );
   asset total_CreateCoin( 0, CreateCoin_SYMBOL );
   asset total_sbd( 0, SBD_SYMBOL );
   asset total_vests( 0, VESTS_SYMBOL );

   asset vesting_shares_CreateCoin_value = asset( 0, CreateCoin_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_CreateCoin += null_account.balance;
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_CreateCoin += null_account.savings_balance;
   }
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      vesting_shares_CreateCoin_value = null_account.vesting_shares * gpo.get_vesting_share_price();
      total_CreateCoin += vesting_shares_CreateCoin_value;
      total_vests += null_account.vesting_shares;
   }

   if( null_account.reward_CreateCoin_balance.amount > 0 )
   {
      total_CreateCoin += null_account.reward_CreateCoin_balance;
   }
   ...
   {
      total_CreateCoin += null_account.reward_vesting_CreateCoin;
      total_vests += null_account.reward_vesting_balance;
   }

   if( (total_CreateCoin.amount.value == 0) && (total_sbd.amount.value == 0) && (total_vests.amount.value == 0) )
      return;
   ...
   if( total_CreateCoin.amount.value > 0 )
      vop.total_cleared.push_back( total_CreateCoin );
   ...
   if( null_account.vesting_shares.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.total_vesting_fund_CreateCoin -= vesting_shares_CreateCoin_value;
      });
   ...
   if( null_account.reward_CreateCoin_balance.amount > 0 )
   {
      adjust_reward_balance( null_account, -null_account.reward_CreateCoin_balance );
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.pending_rewarded_vesting_CreateCoin -= null_account.reward_vesting_CreateCoin;
      });

      modify( null_account, [&]( account_object& a )
      {
         a.reward_vesting_CreateCoin.amount = 0;
   ...
   if( total_CreateCoin.amount > 0 )
      adjust_supply( -total_CreateCoin );
...
{
   if( head_block_num() >= CreateCoin_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_CreateCoin = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_CreateCoin_converted = asset( 0, CreateCoin_SYMBOL );

      // Do two passes, the first for vests, the second for CreateCoin. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / CreateCoin_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / CreateCoin_100_PERCENT ).to_uint64();
            vests_deposited_as_CreateCoin += to_deposit;
            auto converted_CreateCoin = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_CreateCoin_converted += converted_CreateCoin;

            if( to_deposit > 0 )
            {
               operation vop = fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_CreateCoin );

               pre_push_virtual_operation( vop );

               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_CreateCoin;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_CreateCoin -= converted_CreateCoin;
      ...
      share_type to_convert = to_withdraw - vests_deposited_as_CreateCoin - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_CreateCoin = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      operation vop = fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_CreateCoin );
      pre_push_virtual_operation( vop );
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( CreateCoin_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         for( auto& item : proxy_set )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               create_vesting2( *this, voter, asset( claim, CreateCoin_SYMBOL ), has_hardfork( CreateCoin_HARDFORK_0_17__659 ),
                  [&]( const asset& reward )
                  {
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment, bool forward_curation_remainder )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( CreateCoin_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / CreateCoin_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / CreateCoin_100_PERCENT;
               auto benefactor_vesting_CreateCoin = benefactor_tokens;
               auto vop = comment_benefactor_reward_operation( b.account, comment.author, to_string( comment.permlink ), asset( 0, SBD_SYMBOL ), asset( 0, CreateCoin_SYMBOL ), asset( 0, VESTS_SYMBOL ) );

               if( has_hardfork( CreateCoin_HARDFORK_0_20__2022 ) )
               {
                  auto benefactor_sbd_CreateCoin = ( benefactor_tokens * comment.percent_CreateCoin_dollars ) / ( 2 * CreateCoin_100_PERCENT ) ;
                  benefactor_vesting_CreateCoin  = benefactor_tokens - benefactor_sbd_CreateCoin;
                  auto sbd_payout           = create_sbd( get_account( b.account ), asset( benefactor_sbd_CreateCoin, CreateCoin_SYMBOL ), true );

                  vop.sbd_payout   = sbd_payout.first; // SBD portion
                  vop.CreateCoin_payout = sbd_payout.second; // CreateCoin portion
               }

               create_vesting2( *this, get_account( b.account ), asset( benefactor_vesting_CreateCoin, CreateCoin_SYMBOL ), has_hardfork( CreateCoin_HARDFORK_0_17__659 ),
               [&]( const asset& reward )
               {
            ...
            auto sbd_CreateCoin     = ( author_tokens * comment.percent_CreateCoin_dollars ) / ( 2 * CreateCoin_100_PERCENT ) ;
            auto vesting_CreateCoin = author_tokens - sbd_CreateCoin;

            const auto& author = get_account( comment.author );
            auto sbd_payout = create_sbd( author, asset( sbd_CreateCoin, CreateCoin_SYMBOL ), has_hardfork( CreateCoin_HARDFORK_0_17__659 ) );
            ...
            create_vesting2( *this, author, asset( vesting_CreateCoin, CreateCoin_SYMBOL ), has_hardfork( CreateCoin_HARDFORK_0_17__659 ),
            ...
            adjust_total_payout( comment, sbd_payout.first + to_sbd( sbd_payout.second + asset( vesting_CreateCoin, CreateCoin_SYMBOL ) ), to_sbd( asset( curation_tokens, CreateCoin_SYMBOL ) ), to_sbd( asset( total_beneficiary, CreateCoin_SYMBOL ) ) );

            post_push_virtual_operation( vop );
            vop = comment_reward_operation( comment.author, to_string( comment.permlink ), to_sbd( asset( claimed_reward, CreateCoin_SYMBOL ) ) );
            pre_push_virtual_operation( vop );
            post_push_virtual_operation( vop );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( CreateCoin_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == CreateCoin_ROOT_POST_PARENT )
         {
            if( has_hardfork( CreateCoin_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + CreateCoin_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( CreateCoin_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( CreateCoin_FIRST_CASHOUT_TIME ) )
      return;

   const auto& gpo = get_dynamic_global_properties();
   util::comment_reward_context ctx;
   ctx.current_CreateCoin_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > CreateCoin_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( CreateCoin_HARDFORK_0_19__1051 ) )
            decay_time = CreateCoin_RECENT_RSHARES_DECAY_TIME_HF19;
         else
            decay_time = CreateCoin_RECENT_RSHARES_DECAY_TIME_HF17;
   ...
   if( has_hardfork( CreateCoin_HARDFORK_0_17__771 ) )
   {
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( CreateCoin_HARDFORK_0_17__771 ) )
      {
         ...
         ctx.total_reward_fund_CreateCoin = funds[ fund_id ].reward_balance;

         bool forward_curation_remainder = !has_hardfork( CreateCoin_HARDFORK_0_20__1877 );

         funds[ fund_id ].CreateCoin_awarded += cashout_comment_helper( ctx, *current, forward_curation_remainder );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.total_reward_fund_CreateCoin = gpo.total_reward_fund_CreateCoin;
            ...
            if( reward > 0 )
            {
               ...
               {
                  p.total_reward_fund_CreateCoin.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= asset( funds[ i ].CreateCoin_awarded, CreateCoin_SYMBOL );
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual CreateCoin per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( CreateCoin_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( CreateCoin_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / CreateCoin_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( CreateCoin_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_CreateCoin = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( CreateCoin_100_PERCENT ) * int64_t( CreateCoin_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_CreateCoin * CreateCoin_CONTENT_REWARD_PERCENT ) / CreateCoin_100_PERCENT;
      if( has_hardfork( CreateCoin_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
      auto vesting_reward = ( new_CreateCoin * CreateCoin_VESTING_FUND_PERCENT ) / CreateCoin_100_PERCENT; /// 15% to vesting fund
      auto witness_reward = new_CreateCoin - content_reward - vesting_reward; /// Remaining 10% to witness pay

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= CreateCoin_MAX_WITNESSES;
      ...
      new_CreateCoin = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_CreateCoin += asset( vesting_reward, CreateCoin_SYMBOL );
         if( !has_hardfork( CreateCoin_HARDFORK_0_17__774 ) )
            p.total_reward_fund_CreateCoin  += asset( content_reward, CreateCoin_SYMBOL );
         p.current_supply           += asset( new_CreateCoin, CreateCoin_SYMBOL );
         p.virtual_supply           += asset( new_CreateCoin, CreateCoin_SYMBOL );
      });

      operation vop = producer_reward_operation( cwit.owner, asset( 0, VESTS_SYMBOL ) );
      create_vesting2( *this, get_account( cwit.owner ), asset( witness_reward, CreateCoin_SYMBOL ), false,
   ...
   else
   {
      ...
      if( props.head_block_number < CreateCoin_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_CreateCoin += vesting_reward;
          p.total_reward_fund_CreateCoin  += content_reward;
...
void process_smt_objects_internal( database* db, CreateCoin::chain::smt_phase phase )
{
...
{
   if( has_hardfork( CreateCoin_HARDFORK_0_12__178 ) )
      return asset( 0, CreateCoin_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( CreateCoin_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< CreateCoin_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), CreateCoin_SYMBOL );
   return std::max( percent, CreateCoin_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CreateCoin_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CreateCoin_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), CreateCoin_SYMBOL );
   return std::max( percent, CreateCoin_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CreateCoin_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CreateCoin_CURATE_APR_PERCENT >( props.virtual_supply.amount ), CreateCoin_SYMBOL);
   return std::max( percent, CreateCoin_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CreateCoin_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CreateCoin_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), CreateCoin_SYMBOL);
   auto pay = std::max( percent, CreateCoin_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= CreateCoin_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) )
   {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least CreateCoin_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < CreateCoin_MAX_WITNESSES && props.head_block_number < CreateCoin_START_VESTING_BLOCK )
      return asset( 0, CreateCoin_SYMBOL );
#endif

   static_assert( CreateCoin_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( CreateCoin_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< CreateCoin_POW_APR_PERCENT >( props.virtual_supply.amount ), CreateCoin_SYMBOL);
   return std::max( percent, CreateCoin_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % CreateCoin_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( CreateCoin_HARDFORK_0_8__116 ) )
      return CreateCoin_1_PERCENT * 25;
   else
      return CreateCoin_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / CreateCoin_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, CreateCoin_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more CreateCoin than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from CreateCoin/sbd at the
...
void database::process_conversions()
{
   ...
   asset net_CreateCoin( 0, CreateCoin_SYMBOL );
   ...
   while( itr != request_by_date.end() && itr->conversion_date <= now )
   {
      ...
      net_sbd   += itr->amount;
      net_CreateCoin += amount_to_issue;
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_CreateCoin;
       p.current_sbd_supply -= net_sbd;
       p.virtual_supply += net_CreateCoin;
       p.virtual_supply -= net_sbd * get_feed_history().current_median_history;
   } );
}

asset database::to_sbd( const asset& CreateCoin )const
{
   return util::to_sbd( get_feed_history().current_median_history, CreateCoin );
}

asset database::to_CreateCoin( const asset& sbd )const
{
   return util::to_CreateCoin( get_feed_history().current_median_history, sbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + CreateCoin_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( old_escrow.from, old_escrow.CreateCoin_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, CreateCoin_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < CreateCoin_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = CreateCoin_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(CreateCoin_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = CreateCoin_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CreateCoin_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = CreateCoin_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CreateCoin_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = CreateCoin_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CreateCoin_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < CreateCoin_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = CreateCoin_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, CreateCoin_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = CreateCoin_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = CreateCoin_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = CreateCoin_INIT_MINER_NAME;
         p.time = CreateCoin_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, CreateCoin_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = CreateCoin_MAX_BLOCK_SIZE;
         p.reverse_auction_seconds = CreateCoin_REVERSE_AUCTION_WINDOW_SECONDS_HF6;
         p.sbd_stop_percent = CreateCoin_SBD_STOP_PERCENT_HF14;
         p.sbd_start_percent = CreateCoin_SBD_START_PERCENT_HF14;
      } );
      ...
      {
         hpo.processed_hardforks.push_back( CreateCoin_GENESIS_TIME );
      } );
      ...
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         ...
         wso.current_shuffled_witnesses[0] = CreateCoin_INIT_MINER_NAME;
         util::rd_syCC_params account_subsidy_syCC_params;
         account_subsidy_syCC_params.resource_unit = CreateCoin_ACCOUNT_SUBSIDY_PRECISION;
         account_subsidy_syCC_params.decay_per_time_unit_denom_shift = CreateCoin_RD_DECAY_DENOM_SHIFT;
         ...
         w_budget = (w_budget * CreateCoin_WITNESS_SUBSIDY_BUDGET_PERCENT) / CreateCoin_100_PERCENT;
         w_budget = std::min( w_budget, int64_t(std::numeric_limits<int32_t>::max()) );
         uint64_t w_decay = wso.median_props.account_subsidy_decay;
         w_decay = (w_decay * CreateCoin_WITNESS_SUBSIDY_DECAY_PERCENT) / CreateCoin_100_PERCENT;
         ...
         util::rd_setup_dynamics_params( account_subsidy_user_params, account_subsidy_syCC_params, wso.account_subsidy_rd );
         util::rd_setup_dynamics_params( account_subsidy_per_witness_user_params, account_subsidy_syCC_params, wso.account_subsidy_witness_rd );
      } );
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      CreateCoin_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         CreateCoin_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
   ...
   if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( CreateCoin_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / CreateCoin_100_PERCENT ).to_uint64() ) )
   {
      uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / CreateCoin_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
   ...
   if( BOOST_UNLIKELY( next_block_num == 1 ) )
   {
      ...
      uint32_t n;
      for( n=0; n<CreateCoin_NUM_HARDFORKS; n++ )
      {
   ...
   if( has_hardfork( CreateCoin_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < CreateCoin_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",CreateCoin_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( CreateCoin_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
struct process_header_visitor
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( CreateCoin_SMT_HARDFORK ), "Automated actions are not enabled until SMT hardfork." );
...
try {
   if( (head_block_num() % CreateCoin_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( CreateCoin_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_sbd_exchange_update + CreateCoin_MAX_FEED_AGE_SECONDS
            && !wit.sbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.sbd_exchange_rate );
         }
      }
      else if( wit.last_sbd_exchange_update < now + CreateCoin_MAX_FEED_AGE_SECONDS &&
          !wit.sbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.sbd_exchange_rate );
      }
   ...
   if( feeds.size() >= CreateCoin_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t CreateCoin_feed_history_window = CreateCoin_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( CreateCoin_HARDFORK_0_16__551) )
            CreateCoin_feed_history_window = CreateCoin_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > CreateCoin_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( CreateCoin_HARDFORK_0_14__230 ) )
            {
               // This block limits the effective median price to force SBD to remain at or
               // below 10% of the combined market cap of CreateCoin and SBD.
               //
               // For example, if we have 500 CreateCoin and 100 SBD, the price is limited to
               // 900 SBD / 500 CreateCoin which works out to be $1.80.  At this price, 500 CreateCoin
...
{ try {
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, CreateCoin_MAX_SIG_CHECK_DEPTH,
            has_hardfork( CreateCoin_HARDFORK_0_20 ) || is_producing() ? CreateCoin_MAX_AUTHORITY_MEMBERSHIP : 0,
            has_hardfork( CreateCoin_HARDFORK_0_20 ) || is_producing() ? CreateCoin_MAX_SIG_CHECK_ACCOUNTS : 0,
            has_hardfork( CreateCoin_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         CreateCoin_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      CreateCoin_ASSERT( trx.expiration <= now + fc::seconds(CreateCoin_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",CreateCoin_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( CreateCoin_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         CreateCoin_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      CreateCoin_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
   ...
   while( true )
   {
      ...
      if( actions_itr == actions.end() )
      {
         ...
         if( pending_itr != pending_action_idx.end() && pending_itr->execution_time <= head_block_time() )
         {
            ...
            const auto& gpo = get_dynamic_global_properties();
            uint64_t required_actions_partition_size = ( gpo.maximum_block_size * gpo.required_actions_partition_percent ) / CreateCoin_100_PERCENT;
...
FC_TODO( "#ifndef not needed after HF 20 is live" );
#ifndef IS_TEST_NET
               if( has_hardfork( CreateCoin_HARDFORK_0_14__278 ) && !has_hardfork( CreateCoin_HARDFORK_0_20__SP190 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > CreateCoin_BLOCKS_PER_DAY )
                  {
   ...
   {
      CreateCoin_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < CreateCoin_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",CreateCoin_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, CreateCoin_SYMBOL ) : dgp.current_sbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( CreateCoin_HARDFORK_0_14__230 ) )
      {
         auto percent_sbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_sbd_supply * get_feed_history().current_median_history ).amount.value ) * CreateCoin_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_sbd <= dgp.sbd_start_percent )
            dgp.sbd_print_rate = CreateCoin_100_PERCENT;
         else if( percent_sbd >= dgp.sbd_stop_percent )
            dgp.sbd_print_rate = 0;
         else
            dgp.sbd_print_rate = ( ( dgp.sbd_stop_percent - percent_sbd ) * CreateCoin_100_PERCENT ) / ( dgp.sbd_stop_percent - dgp.sbd_start_percent );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < CreateCoin_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > CreateCoin_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - CreateCoin_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( CreateCoin_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((CreateCoin_100_PERCENT - CreateCoin_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / CreateCoin_100_PERCENT);
...
{
   bool has_hf_20__1815 = has_hardfork( CreateCoin_HARDFORK_0_20__1815 );

#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      CreateCoin_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
      ...
      CreateCoin_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      CreateCoin_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      CreateCoin_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      CreateCoin_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   }
...
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      CreateCoin_ASSERT( new_order_pays == new_order.amount_for_sale() ||
   ...
   if( !has_hardfork( CreateCoin_HARDFORK_0_12__178 ) &&
       ( (age >= CreateCoin_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( CreateCoin_HARDFORK_0_10__149)) ||
       (age >= CreateCoin_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( CreateCoin_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == CreateCoin_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= CreateCoin_LIQUIDITY_TIMEOUT_SEC )
         {
            r.sbd_volume = 0;
            r.CreateCoin_volume = 0;
            r.weight = 0;
         }
         ...
         if( is_sdb )
            ...
            r.CreateCoin_volume += volume.amount.value;

         r.update_weight( has_hardfork( CreateCoin_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( CreateCoin_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
   try
   {
      CreateCoin_ASSERT( order.amount_for_sale().symbol == pays.symbol,
         order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
         ("order", order)("pays", pays)("receives", receives) );
      CreateCoin_ASSERT( pays.symbol != receives.symbol,
         order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
         ("order", order)("pays", pays)("receives", receives) );
      ...
      {
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
         if( has_hardfork( CreateCoin_HARDFORK_0_20__1815 ) )
         {
            CreateCoin_ASSERT( pays < order.amount_for_sale(),
              order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
              ("order", order)("pays", pays)("receives", receives) );
         }
...
void database::clear_expired_delegations()
{
   ...
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      ...
      {
         if( has_hardfork( CreateCoin_HARDFORK_0_20__2539 ) )
         {
            util::manabar_params params( util::get_effective_vesting_shares( a ), CreateCoin_VOTING_MANA_REGENERATION_SECONDS );
FC_TODO( "Set skip_cap_regen=true without breaking consensus" );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case CreateCoin_ASSET_NUM_CreateCoin:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.balance.amount.value >= 0, "Insufficient CreateCoin funds" );
            }
            break;
         case CreateCoin_ASSET_NUM_SBD:
            if( a.sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.sbd_seconds > 0 &&
                   (acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment).to_seconds() > CreateCoin_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.sbd_seconds / CreateCoin_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= CreateCoin_100_PERCENT;
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( value_delta.symbol.asset_num )
      {
         case CreateCoin_ASSET_NUM_CreateCoin:
            ...
            {
               acnt.reward_CreateCoin_balance += value_delta;
               if( check_balance )
               {
                  FC_ASSERT( acnt.reward_CreateCoin_balance.amount.value >= 0, "Insufficient reward CreateCoin funds" );
               }
            }
            else
            {
               acnt.reward_vesting_CreateCoin += value_delta;
...
{
   bool check_balance = has_hardfork( CreateCoin_HARDFORK_0_20__1811 );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case CreateCoin_ASSET_NUM_CreateCoin:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_balance.amount.value >= 0, "Insufficient savings CreateCoin funds" );
            }
            break;
         case CreateCoin_ASSET_NUM_SBD:
            if( a.savings_sbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_sbd_seconds > 0 &&
                   (acnt.savings_sbd_seconds_last_update - acnt.savings_sbd_last_interest_payment).to_seconds() > CreateCoin_SBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_sbd_seconds / CreateCoin_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().sbd_interest_rate;
                  interest /= CreateCoin_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), SBD_SYMBOL);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   bool check_supply = has_hardfork( CreateCoin_HARDFORK_0_20__1811 );

   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < CreateCoin_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol.asset_num )
      {
         case CreateCoin_ASSET_NUM_CreateCoin:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, CreateCoin_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_CreateCoin += new_vesting;
            if( check_supply )
            {
...
void database::generate_optional_actions()
{
   ...
   CreateCoin_TRY_NOTIFY( _generate_optional_actions_signal, note );
}

void database::init_hardforks()
{
   _hardfork_times[ 0 ] = fc::time_point_sec( CreateCoin_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( CreateCoin_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_1 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_1_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_1 ] = CreateCoin_HARDFORK_0_1_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_2 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_2_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_2 ] = CreateCoin_HARDFORK_0_2_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_3 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_3_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_3 ] = CreateCoin_HARDFORK_0_3_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_4 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_4_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_4 ] = CreateCoin_HARDFORK_0_4_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_5 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_5_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_5 ] = CreateCoin_HARDFORK_0_5_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_6 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_6_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_6 ] = CreateCoin_HARDFORK_0_6_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_7 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_7_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_7 ] = CreateCoin_HARDFORK_0_7_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_8 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_8_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_8 ] = CreateCoin_HARDFORK_0_8_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_9 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_9_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_9 ] = CreateCoin_HARDFORK_0_9_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_10 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_10_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_10 ] = CreateCoin_HARDFORK_0_10_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_11 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_11_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_11 ] = CreateCoin_HARDFORK_0_11_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_12 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_12_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_12 ] = CreateCoin_HARDFORK_0_12_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_13 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_13_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_13 ] = CreateCoin_HARDFORK_0_13_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_14 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_14_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_14 ] = CreateCoin_HARDFORK_0_14_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_15 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_15_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_15 ] = CreateCoin_HARDFORK_0_15_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_16 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_16_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_16 ] = CreateCoin_HARDFORK_0_16_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_17 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_17_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_17 ] = CreateCoin_HARDFORK_0_17_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_18 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_18_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_18 ] = CreateCoin_HARDFORK_0_18_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_19 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_19_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_19 ] = CreateCoin_HARDFORK_0_19_VERSION;
   FC_ASSERT( CreateCoin_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_20 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_20_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_20 ] = CreateCoin_HARDFORK_0_20_VERSION;
#ifdef IS_TEST_NET
   FC_ASSERT( CreateCoin_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
   _hardfork_times[ CreateCoin_HARDFORK_0_21 ] = fc::time_point_sec( CreateCoin_HARDFORK_0_21_TIME );
   _hardfork_versions[ CreateCoin_HARDFORK_0_21 ] = CreateCoin_HARDFORK_0_21_VERSION;
#endif


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= CreateCoin_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("CreateCoin_NUM_HARDFORKS",CreateCoin_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= CreateCoin_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( CreateCoin_BLOCKCHAIN_HARDFORK_VERSION >= CreateCoin_BLOCKCHAIN_VERSION );
   FC_ASSERT( CreateCoin_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ CreateCoin_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( CreateCoin_HARDFORK_0_5__54 ) )
      {
         while( _hardfork_versions[ hardforks.last_hardfork ] < hardforks.next_hardfork
            && hardforks.next_hardfork_time <= head_block_time() )
         {
            if( hardforks.last_hardfork < CreateCoin_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < CreateCoin_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < CreateCoin_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= CreateCoin_NUM_HARDFORKS; i++ )
   {
      if( i <= CreateCoin_HARDFORK_0_5__54 )
...
void database::apply_hardfork( uint32_t hardfork )
{
   ...
   switch( hardfork )
   {
      case CreateCoin_HARDFORK_0_1:
         perform_vesting_share_split( 1000000 );
         break;
      case CreateCoin_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case CreateCoin_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case CreateCoin_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case CreateCoin_HARDFORK_0_5:
         break;
      case CreateCoin_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case CreateCoin_HARDFORK_0_7:
         break;
      case CreateCoin_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case CreateCoin_HARDFORK_0_9:
         {
         ...
         break;
      case CreateCoin_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case CreateCoin_HARDFORK_0_11:
         break;
      case CreateCoin_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == CreateCoin_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + CreateCoin_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + CreateCoin_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( CreateCoin_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( CreateCoin_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( CreateCoin_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case CreateCoin_HARDFORK_0_13:
         break;
      case CreateCoin_HARDFORK_0_14:
         break;
      case CreateCoin_HARDFORK_0_15:
         break;
      case CreateCoin_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > CreateCoin_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case CreateCoin_HARDFORK_0_17:
         {
            static_assert(
               CreateCoin_MAX_VOTED_WITNESSES_HF0 + CreateCoin_MAX_MINER_WITNESSES_HF0 + CreateCoin_MAX_RUNNER_WITNESSES_HF0 == CreateCoin_MAX_WITNESSES,
               "HF0 witness counts must add up to CreateCoin_MAX_WITNESSES" );
            static_assert(
               CreateCoin_MAX_VOTED_WITNESSES_HF17 + CreateCoin_MAX_MINER_WITNESSES_HF17 + CreateCoin_MAX_RUNNER_WITNESSES_HF17 == CreateCoin_MAX_WITNESSES,
               "HF17 witness counts must add up to CreateCoin_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = CreateCoin_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = CreateCoin_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = CreateCoin_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = CreateCoin_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = CreateCoin_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = CreateCoin_1_PERCENT * 25;
               rfo.percent_content_rewards = CreateCoin_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_CreateCoin;
#ifndef IS_TEST_NET
               rfo.recent_claims = CreateCoin_HF_17_RECENT_CLAIMS;
#endif
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_CreateCoin = asset( 0, CreateCoin_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( CreateCoin_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( CreateCoin_HF_17_NUM_REPLIES );
            ...
            for( const auto& itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( head_block_time() + CreateCoin_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( const auto& itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), head_block_time() + CreateCoin_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case CreateCoin_HARDFORK_0_18:
         break;
      case CreateCoin_HARDFORK_0_19:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.vote_power_reserve_rate = CreateCoin_REDUCED_VOTE_POWER_RATE;
            });

            modify( get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = CreateCoin_HF_19_RECENT_CLAIMS;
#endif
         ...
         break;
      case CreateCoin_HARDFORK_0_20:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.delegation_return_period = CreateCoin_DELEGATION_RETURN_PERIOD_HF20;
               gpo.reverse_auction_seconds = CreateCoin_REVERSE_AUCTION_WINDOW_SECONDS_HF20;
               gpo.sbd_stop_percent = CreateCoin_SBD_STOP_PERCENT_HF20;
               gpo.sbd_start_percent = CreateCoin_SBD_START_PERCENT_HF20;
            ...
            for( const auto& witness : wso.current_shuffled_witnesses )
            {
               ...
               if( witness != account_name_type() )
               {
                  ...
                  {
                     w.props.account_creation_fee = asset( w.props.account_creation_fee.amount * CreateCoin_CREATE_ACCOUNT_WITH_CreateCoin_MODIFIER, CreateCoin_SYMBOL );
                  });
               }
            }
            ...
            {
               wso.median_props.account_creation_fee = asset( wso.median_props.account_creation_fee.amount * CreateCoin_CREATE_ACCOUNT_WITH_CreateCoin_MODIFIER, CreateCoin_SYMBOL );
            });
         }
         break;
      case CreateCoin_SMT_HARDFORK:
      {
#ifdef CreateCoin_ENABLE_SMT
         ...
         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.required_actions_partition_percent = 25 * CreateCoin_1_PERCENT;
         });
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, CreateCoin_SYMBOL );
      asset total_sbd = asset( 0, SBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_CreateCoin = asset( 0, CreateCoin_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_CreateCoin_balance;
         ...
         pending_vesting_CreateCoin += itr->reward_vesting_CreateCoin;
         total_vsf_votes += ( itr->proxy == CreateCoin_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( CreateCoin_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[CreateCoin_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == CreateCoin_SYMBOL )
      ...
      {
         if( itr->sell_price.base.symbol == CreateCoin_SYMBOL )
         {
            total_supply += asset( itr->for_sale, CreateCoin_SYMBOL );
         }
      ...
      {
         total_supply += itr->CreateCoin_balance;
         total_sbd += itr->sbd_balance;

         if( itr->pending_fee.symbol == CreateCoin_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == SBD_SYMBOL )
            total_sbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not SBD or CreateCoin" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == CreateCoin_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not SBD or CreateCoin" );
      }
      ...
      total_supply += gpo.total_vesting_fund_CreateCoin + gpo.total_reward_fund_CreateCoin + gpo.pending_rewarded_vesting_CreateCoin;
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_CreateCoin == pending_vesting_CreateCoin, "", ("pending_rewarded_vesting_CreateCoin",gpo.pending_rewarded_vesting_CreateCoin)("pending_vesting_CreateCoin", pending_vesting_CreateCoin));
...
void database::validate_smt_invariants()const
{
   try
   {
      ...
      // - Process reward balances, collecting SMT counterparts of 'reward_CreateCoin_balance', 'reward_vesting_balance' & 'reward_vesting_CreateCoin'.
...
#pragma message( "TODO: Add reward_fund_object iteration here once they support SMTs." )
      ...
      for( ; itr != end; ++itr )
      {
         ...
         total_liquid_supply += asset( smt.total_vesting_fund_smt, smt.liquid_symbol )
                             /*+ gpo.total_reward_fund_CreateCoin */
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / CreateCoin_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < CreateCoin_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != CreateCoin_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != CreateCoin_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != CreateCoin_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.name )
