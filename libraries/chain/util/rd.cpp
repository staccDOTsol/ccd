...
{
   FC_ASSERT( user_params.budget_per_time_unit >= CreateCoin_RD_MIN_BUDGET );
   FC_ASSERT( user_params.budget_per_time_unit <= CreateCoin_RD_MAX_BUDGET );
   FC_ASSERT( user_params.decay_per_time_unit >= CreateCoin_RD_MIN_DECAY );
   FC_ASSERT( user_params.decay_per_time_unit <= CreateCoin_RD_MAX_DECAY );
}
...
{
   dparams_out.resource_unit = syCC_params.resource_unit;
   ...
   dparams_out.decay_params.decay_per_time_unit_denom_shift = syCC_params.decay_per_time_unit_denom_shift;

   //
   // Following assert should never trigger, as decay_per_time_unit_denom_shift is a syCC param and should
   //   not be user settable.  SyCC code which sets it should just set it to
   //   CreateCoin_RD_DECAY_DENOM_SHIFT.  The interface is designed to support other values of this
   ...
   //   code so that it sets decay_per_time_unit_denom_shift = CreateCoin_RD_DECAY_DENOM_SHIFT,
   ...
   //
   FC_ASSERT( syCC_params.decay_per_time_unit_denom_shift == CreateCoin_RD_DECAY_DENOM_SHIFT );
   ...
   static_assert( CreateCoin_RD_MAX_BUDGET < (uint64_t(1) << (64-CreateCoin_RD_DECAY_DENOM_SHIFT)),
      "Computation of temp could overflow here, set smaller CreateCoin_RD_MAX_BUDGET" );

   fc::uint128_t temp = user_params.budget_per_time_unit;
   temp <<= syCC_params.decay_per_time_unit_denom_shift;
