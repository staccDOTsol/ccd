...
{
   block_notification( const CreateCoin::protocol::signed_block& b ) : block(b)
   {
      block_id = b.id();
      block_num = block_header::num_from_id( block_id );
   }

   CreateCoin::protocol::block_id_type          block_id;
   uint32_t                                block_num = 0;
   const CreateCoin::protocol::signed_block&    block;
};
