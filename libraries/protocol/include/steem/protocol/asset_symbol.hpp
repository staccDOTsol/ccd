...
#define CreateCoin_ASSET_SYMBOL_PRECISION_BITS    4
#define CreateCoin_ASSET_CONTROL_BITS             1
#define CreateCoin_NAI_SHIFT                      ( CreateCoin_ASSET_SYMBOL_PRECISION_BITS + CreateCoin_ASSET_CONTROL_BITS )
#define SMT_MAX_NAI                          99999999
#define SMT_MIN_NAI                          1
#define SMT_MIN_NON_RESERVED_NAI             10000000
#define CreateCoin_ASSET_SYMBOL_NAI_LENGTH        10
#define CreateCoin_ASSET_SYMBOL_NAI_STRING_LENGTH ( CreateCoin_ASSET_SYMBOL_NAI_LENGTH + 2 )
#define SMT_MAX_NAI_POOL_COUNT               10
#define SMT_MAX_NAI_GENERATION_TRIES         100

#define CreateCoin_PRECISION_SBD   (3)
#define CreateCoin_PRECISION_CreateCoin (3)
#define CreateCoin_PRECISION_VESTS (6)
...
#define CreateCoin_NAI_SBD   (1)
#define CreateCoin_NAI_CreateCoin (2)
#define CreateCoin_NAI_VESTS (3)

#define CreateCoin_ASSET_NUM_SBD \
  (uint32_t(((SMT_MAX_NAI + CreateCoin_NAI_SBD)   << CreateCoin_NAI_SHIFT) | CreateCoin_PRECISION_SBD))
#define CreateCoin_ASSET_NUM_CreateCoin \
  (uint32_t(((SMT_MAX_NAI + CreateCoin_NAI_CreateCoin) << CreateCoin_NAI_SHIFT) | CreateCoin_PRECISION_CreateCoin))
#define CreateCoin_ASSET_NUM_VESTS \
  (uint32_t(((SMT_MAX_NAI + CreateCoin_NAI_VESTS) << CreateCoin_NAI_SHIFT) | CreateCoin_PRECISION_VESTS))

#ifdef IS_TEST_NET

#define VESTS_SYMBOL_U64  (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define CreateCoin_SYMBOL_U64  (uint64_t('T') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define SBD_SYMBOL_U64    (uint64_t('T') | (uint64_t('B') << 8) | (uint64_t('D') << 16))

#else

#define VESTS_SYMBOL_U64  (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define CreateCoin_SYMBOL_U64  (uint64_t('S') | (uint64_t('T') << 8) | (uint64_t('E') << 16) | (uint64_t('E') << 24) | (uint64_t('M') << 32))
...
#define CreateCoin_SYMBOL_SER  (uint64_t(3) | (CreateCoin_SYMBOL_U64 << 8)) ///< CreateCoin|TESTS with 3 digits of precision
#define SBD_SYMBOL_SER    (uint64_t(3) |   (SBD_SYMBOL_U64 << 8)) ///< SBD|TBD with 3 digits of precision

#define CreateCoin_ASSET_MAX_DECIMALS 12
...
class asset_symbol_type
{
   public:
      ...
      // buf must have space for CreateCoin_ASSET_SYMBOL_MAX_LENGTH+1
      ...
      {
         char buf[ CreateCoin_ASSET_SYMBOL_NAI_STRING_LENGTH ];
         to_nai_string( buf );
...
FC_REFLECT(CreateCoin::protocol::asset_symbol_type, (asset_num))
...
inline void pack( Stream& s, const CreateCoin::protocol::asset_symbol_type& sym )
{
   switch( sym.space() )
   {
      case CreateCoin::protocol::asset_symbol_type::legacy_space:
      {
         uint64_t ser = 0;
         switch( sym.asset_num )
         {
            case CreateCoin_ASSET_NUM_CreateCoin:
               ser = CreateCoin_SYMBOL_SER;
               break;
      ...
      case CreateCoin::protocol::asset_symbol_type::smt_nai_space:
...
inline void unpack( Stream& s, CreateCoin::protocol::asset_symbol_type& sym )
{
   ...
   switch( ser )
   {
      case CreateCoin_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == CreateCoin_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = CreateCoin_ASSET_NUM_CreateCoin;
         break;
      case SBD_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == SBD_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = CreateCoin_ASSET_NUM_SBD;
         break;
      case VESTS_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == VESTS_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = CreateCoin_ASSET_NUM_VESTS;
         break;
      default:
...
inline void to_variant( const CreateCoin::protocol::asset_symbol_type& sym, fc::variant& var )
{
   try
   {
...
inline void from_variant( const fc::variant& var, CreateCoin::protocol::asset_symbol_type& sym )
{
   ...
   try
   {
      ...
      FC_ASSERT( decimals->value().as_uint64() <= CreateCoin_ASSET_MAX_DECIMALS,
         "Expected decimals to be less than or equal to ${num}", ("num", CreateCoin_ASSET_MAX_DECIMALS) );
