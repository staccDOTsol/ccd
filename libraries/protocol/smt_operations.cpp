...
void smt_create_operation::validate()const
{
   ...
   FC_ASSERT( smt_creation_fee.amount <= CreateCoin_MAX_SHARE_SUPPLY, "Fee must be smaller than CreateCoin_MAX_SHARE_SUPPLY" );
   FC_ASSERT( is_asset_type( smt_creation_fee, CreateCoin_SYMBOL ) || is_asset_type( smt_creation_fee, SBD_SYMBOL ), "Fee must be CreateCoin or SBD" );
...
uint32_t smt_generation_unit::CreateCoin_unit_sum()const
{
   uint32_t result = 0;
   for(const std::pair< account_name_type, uint16_t >& e : CreateCoin_unit )
      result += e.second;
   return result;
}
...
void smt_capped_generation_policy::validate()const
{
   ...
   FC_ASSERT( pre_soft_cap_unit.CreateCoin_unit.size() > 0 );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() > 0 );

   FC_ASSERT( pre_soft_cap_unit.CreateCoin_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.CreateCoin_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );

   // TODO : Check account name

   if( soft_cap_percent == CreateCoin_100_PERCENT )
   {
      FC_ASSERT( post_soft_cap_unit.CreateCoin_unit.size() == 0 );
   ...
   min_CreateCoin_units_commitment.validate();
   hard_cap_CreateCoin_units_commitment.validate();
   ...
   FC_ASSERT( hard_cap_CreateCoin_units_commitment.lower_bound >= SMT_MIN_HARD_CAP_CreateCoin_UNITS );
   ...
   static_assert( uint64_t( CreateCoin_MAX_SHARE_SUPPLY ) < (std::numeric_limits< uint64_t >::max() / CreateCoin_100_PERCENT), "Overflow check failed" );
   uint64_t min_soft_cap = (uint64_t( hard_cap_CreateCoin_units_commitment.lower_bound.value ) * soft_cap_percent) / CreateCoin_100_PERCENT;
   uint64_t max_soft_cap = (uint64_t( hard_cap_CreateCoin_units_commitment.upper_bound.value ) * soft_cap_percent) / CreateCoin_100_PERCENT;
   ...
   // We want to prevent the following from overflowing CreateCoin_MAX_SHARE_SUPPLY:
   // max_tokens_created = (u1.tt * sc + u2.tt * (hc-sc)) * min_unit_ratio
   // max_CreateCoin_accepted =  u1.st * sc + u2.st * (hc-sc)

   // hc / max_unit_ratio is the saturation point

   uint128_t sc = max_soft_cap;
   uint128_t hc_sc = hard_cap_CreateCoin_units_commitment.upper_bound.value - max_soft_cap;

   uint128_t max_tokens_created = (pre_soft_cap_unit.token_unit_sum() * sc + post_soft_cap_unit.token_unit_sum() * hc_sc) * min_unit_ratio;
   uint128_t max_share_supply_u128 = uint128_t( CreateCoin_MAX_SHARE_SUPPLY );

   FC_ASSERT( max_tokens_created <= max_share_supply_u128 );

   uint128_t max_CreateCoin_accepted = (pre_soft_cap_unit.CreateCoin_unit_sum() * sc + post_soft_cap_unit.CreateCoin_unit_sum() * hc_sc);
...
void smt_setup_emissions_operation::validate()const
{
   ...
   FC_ASSERT( schedule_time > CreateCoin_GENESIS_TIME );
...
struct smt_set_runtime_parameters_operation_visitor
{
   ...
   {
      FC_ASSERT( param_rewards.percent_curation_rewards <= CreateCoin_100_PERCENT,
