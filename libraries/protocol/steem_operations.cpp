...
namespace CreateCoin { namespace protocol {
   ...
   void validate_auth_size( const authority& a )
   {
      ...
      FC_ASSERT( size <= CreateCoin_MAX_AUTHORITY_MEMBERSHIP, "Authority membership exceeded. Max: 10 Current: ${n}", ("n", size) );
   }

   void account_create_operation::validate() const
   {
      validate_account_name( new_account_name );
      FC_ASSERT( is_asset_type( fee, CreateCoin_SYMBOL ), "Account creation fee must be CreateCoin" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, CreateCoin_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= CreateCoin_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= CreateCoin_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= CreateCoin_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_CreateCoin_dollars <= CreateCoin_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void claim_account_operation::validate()const
   {
      ...
      FC_ASSERT( is_asset_type( fee, CreateCoin_SYMBOL ), "Account creation fee must be CreateCoin" );
      FC_ASSERT( fee >= asset( 0, CreateCoin_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( fee <= asset( CreateCoin_MAX_ACCOUNT_CREATION_FEE, CreateCoin_SYMBOL ), "Account creation fee cannot be too large" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= CreateCoin_100_PERCENT, "Weight is not a CreateCoinIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of CreateCoin Power (STMP) is not allowed." );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
      FC_ASSERT( memo.size() < CreateCoin_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= CreateCoin_100_PERCENT, "Percent must be valid CreateCoin percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= CreateCoin_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, CreateCoin_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == CreateCoin_SYMBOL, "account_creation_fee must be in CreateCoin" );
         FC_ASSERT( account_creation_fee.amount >= CreateCoin_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= CreateCoin_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( sbd_interest_rate <= CreateCoin_100_PERCENT, "sbd_interest_rate must not exceed 100%" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( sbd_exchange_rate.base, SBD_SYMBOL ) && is_asset_type( sbd_exchange_rate.quote, CreateCoin_SYMBOL ) ),
            "Price feed must be a CreateCoin/SBD price" );
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= CreateCoin_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", CreateCoin_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= CreateCoin_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", CreateCoin_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= CreateCoin_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", CreateCoin_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= CreateCoin_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", CreateCoin_RD_MAX_DECAY) );
      }
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( CreateCoin_EQUIHASH_N, CreateCoin_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == CreateCoin_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == CreateCoin_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, CreateCoin_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, CreateCoin_SYMBOL ) ),
         "Price feed must be a CreateCoin/SBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, CreateCoin_SYMBOL ) && is_asset_type( min_to_receive, SBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( min_to_receive, CreateCoin_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( min_to_receive, CreateCoin_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, CreateCoin_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
                  ),
               "Limit order must be for the CreateCoin:SBD or SMT:(CreateCoin/SBD) market" );
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, CreateCoin_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, CreateCoin_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( exchange_rate.quote, CreateCoin_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, CreateCoin_SYMBOL )
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from SBD to CreateCoin, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, SBD_SYMBOL ), "Can only convert SBD to CreateCoin" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( CreateCoin_amount.amount >= 0, "CreateCoin amount cannot be negative" );
      FC_ASSERT( sbd_amount.amount > 0 || CreateCoin_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == CreateCoin_SYMBOL) || (fee.symbol == SBD_SYMBOL), "fee must be CreateCoin or SBD" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
      FC_ASSERT( CreateCoin_amount.symbol == CreateCoin_SYMBOL, "CreateCoin amount must contain CreateCoin" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( sbd_amount.amount >= 0, "sbd amount cannot be negative" );
      FC_ASSERT( CreateCoin_amount.amount >= 0, "CreateCoin amount cannot be negative" );
      FC_ASSERT( sbd_amount.amount > 0 || CreateCoin_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
      FC_ASSERT( CreateCoin_amount.symbol == CreateCoin_SYMBOL, "CreateCoin amount must contain CreateCoin" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == CreateCoin_SYMBOL || amount.symbol == SBD_SYMBOL );
      FC_ASSERT( memo.size() < CreateCoin_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_CreateCoin, CreateCoin_SYMBOL ), "Reward CreateCoin must be CreateCoin" );
      FC_ASSERT( is_asset_type( reward_sbd, SBD_SYMBOL ), "Reward CreateCoin must be SBD" );
      FC_ASSERT( is_asset_type( reward_vests, VESTS_SYMBOL ), "Reward CreateCoin must be VESTS" );
      FC_ASSERT( reward_CreateCoin.amount >= 0, "Cannot claim a negative amount" );
      ...
      FC_ASSERT( reward_CreateCoin.amount > 0 || reward_sbd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
   }
