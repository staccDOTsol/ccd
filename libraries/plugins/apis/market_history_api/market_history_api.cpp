...
{
   public:
      market_history_api_impl() : _db( appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >().db() ) {}
...
DEFINE_API_IMPL( market_history_api_impl, get_ticker )
{
   ...
   {
      auto open = ASSET_TO_REAL( asset( itr->non_CreateCoin.open, SBD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->CreateCoin.open, CreateCoin_SYMBOL ) );
      itr = bucket_idx.lower_bound( boost::make_tuple( 0, _db.head_block_time() ) );
      result.latest = ASSET_TO_REAL( asset( itr->non_CreateCoin.close, SBD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->CreateCoin.close, CreateCoin_SYMBOL ) );
   ...
   result.CreateCoin_volume = volume.CreateCoin_volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_volume )
{
   ...
   do
   {
      result.CreateCoin_volume.amount += itr->CreateCoin.volume;
      result.sbd_volume.amount += itr->non_CreateCoin.volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_order_book )
{
   ...
   auto itr = order_idx.lower_bound( price::max( SBD_SYMBOL, CreateCoin_SYMBOL ) );
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == SBD_SYMBOL && result.bids.size() < args.limit )
   {
      ...
      cur.CreateCoin = ( asset( itr->for_sale, SBD_SYMBOL ) * itr->sell_price ).amount;
   ...
   itr = order_idx.lower_bound( price::max( CreateCoin_SYMBOL, SBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == CreateCoin_SYMBOL && result.asks.size() < args.limit )
   {
      order cur;
      cur.order_price = itr->sell_price;
      cur.real_price = ASSET_TO_REAL( itr->sell_price.quote ) / ASSET_TO_REAL( itr->sell_price.base );
      cur.CreateCoin = itr->for_sale;
      cur.sbd = ( asset( itr->for_sale, CreateCoin_SYMBOL ) * itr->sell_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( market_history_api_impl, get_market_history_buckets )
{
   ...
   result.bucket_sizes = appbase::app().get_plugin< CreateCoin::plugins::market_history::market_history_plugin >().get_tracked_buckets();
   return result;
}
...
{
   JSON_RPC_REGISTER_API( CreateCoin_MARKET_HISTORY_API_PLUGIN_NAME );
}
