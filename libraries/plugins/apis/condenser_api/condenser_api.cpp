...
#include <CreateCoin/plugins/account_history_api/account_history_api_plugin.hpp>
#include <CreateCoin/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <CreateCoin/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <CreateCoin/plugins/tags_api/tags_api_plugin.hpp>
#include <CreateCoin/plugins/follow_api/follow_api_plugin.hpp>
#include <CreateCoin/plugins/reputation_api/reputation_api_plugin.hpp>
#include <CreateCoin/plugins/market_history_api/market_history_api_plugin.hpp>
...
namespace detail
{
   ...
   class condenser_api_impl
   {
      public:
         condenser_api_impl() :
            _chain( appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >() ),
            _db( _chain.db() )
         {
            _on_post_apply_block_conn = _db.add_post_apply_block_handler(
               [&]( const block_notification& note ){ on_post_apply_block( note.block ); },
               appbase::app().get_plugin< CreateCoin::plugins::condenser_api::condenser_api_plugin >(),
               0 );
         }
         ...
         CreateCoin::plugins::chain::chain_plugin&                              _chain;
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_version )
   {
      ...
      (
         fc::string( CreateCoin_BLOCKCHAIN_VERSION ),
         fc::string( CreateCoin::utilities::git_revision_sha ),
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored for CreateCoin." );
   }
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      ...
      if( _db.has_hardfork( CreateCoin_HARDFORK_0_17__774 ) )
         ...
         pot = props.total_reward_fund_CreateCoin;
      ...
      if( d.parent_author != CreateCoin_ROOT_POST_PARENT )
...
{
   if( a.voting_manabar.last_update_time < CreateCoin_HARDFORK_0_20_TIME )
   ...
   // vp_t2 = CreateCoin_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - CreateCoin_100_PERCENT * (t2 - t1) / CreateCoin_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = CreateCoin_100_PERCENT * dt / CreateCoin_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = CreateCoin_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= CreateCoin_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to CreateCoin_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = CreateCoin_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( CreateCoin_CONDENSER_API_PLUGIN_NAME );
}
