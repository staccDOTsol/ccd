...
{
   public:
      tags_api_impl() : _db( appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >().db() ) {}
      ...
      chain::database& _db;
      std::shared_ptr< CreateCoin::plugins::follow::follow_api > _follow_api;
};
...
DEFINE_API_IMPL( tags_api_impl, get_discussions_by_promoted )
{
   ...
   auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type( CreateCoin_MAX_SHARE_SUPPLY ) )  );
...
{
   JSON_RPC_REGISTER_API( CreateCoin_TAGS_API_PLUGIN_NAME );
}
...
{
   auto follow_api_plugin = appbase::app().find_plugin< CreateCoin::plugins::follow::follow_api_plugin >();
