...
#include <CreateCoin/plugins/rc/rc_curve.hpp>
#include <CreateCoin/plugins/rc/rc_export_objects.hpp>
...
#include <CreateCoin/jsonball/jsonball.hpp>

#define CreateCoin_RC_REGEN_TIME   (60*60*24*5)
// 2020.748973 VESTS == 1.000 CreateCoin when HF20 occurred on mainnet
// TODO: What should this value be for testnet?
#define CreateCoin_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT      2020748973

#ifndef IS_TEST_NET
#define CreateCoin_HF20_BLOCK_NUM                              26256743
#endif

// 1.66% is ~2 hours of regen.
// 2 / ( 24 * 5 ) = 0.01666...
#define CreateCoin_RC_MAX_NEGATIVE_PERCENT 166

namespace CreateCoin { namespace plugins { namespace rc {

using CreateCoin::plugins::block_data_export::block_data_export_plugin;

namespace detail {

using chain::plugin_exception;
using CreateCoin::chain::util::manabar_params;

class rc_plugin_impl
{
   public:
      rc_plugin_impl( rc_plugin& _plugin ) :
         _db( appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >().db() ),
         _self( _plugin )
      {
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment )
{
   ...
   if( max_rc_creation_adjustment.symbol == CreateCoin_SYMBOL )
   {
   ...
   rc_plugin_skip_flags skip )
{
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         CreateCoin_ASSERT( false, plugin_exception,
   ...
   mbparams.regen_time = CreateCoin_RC_REGEN_TIME;
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( CreateCoin_HARDFORK_0_20 ) )
      {
         if( db.is_producing() )
         {
            CreateCoin_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up CreateCoin.",
      ...
      int64_t min_mana = -1 * ( CreateCoin_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / CreateCoin_100_PERCENT;
...
void rc_plugin_impl::on_post_apply_transaction( const transaction_notification& note )
{
   ...
   int64_t rc_regen = (gpo.total_vesting_shares.amount.value / (CreateCoin_RC_REGEN_TIME / CreateCoin_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<CreateCoin_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      CreateCoin::plugins::block_data_export::find_export_data< exp_rc_data >( CreateCoin_RC_PLUGIN_NAME );
      ...
      [&]( rc_pool_object& pool_obj )
      {
         ...
         for( size_t i=0; i<CreateCoin_NUM_RESOURCE_TYPES; i++ )
         {
            ...
            if( debug_print )
            {
               ...
               a /= k*double(pool);
               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_vesting_shares.amount.value/CreateCoin_RC_REGEN_TIME) );
            }
         }
         if( debug_print )
         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = CreateCoin::jsonball::get_resource_parameters();
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, asset( CreateCoin_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT, VESTS_SYMBOL ) );
   }
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( CreateCoin_RC_REGEN_TIME <= CreateCoin_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
   ...
   {
      if( op.hardfork_id == CreateCoin_HARDFORK_0_1 )
      {
         const auto& idx = _db.get_index< account_index >().indices().get< by_id >();
         for( auto it=idx.begin(); it!=idx.end(); ++it )
         {
   ...
   {
      regenerate( CreateCoin_NULL_ACCOUNT );
   }
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const pow_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, op.worker_account, asset( 0, CreateCoin_SYMBOL ) );
   ...
   void operator()( const pow2_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, worker_name, asset( 0, CreateCoin_SYMBOL ) );
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == CreateCoin_HARDFORK_0_20 )
      {
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.emplace_back( CreateCoin_NULL_ACCOUNT );
   }
...
void rc_plugin_impl::on_pre_apply_operation( const operation_notification& note )
{
   ...
   if( _db.has_hardfork( CreateCoin_HARDFORK_0_20 ) )
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( CreateCoin_RC_PLUGIN_NAME,
            []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_rc_data >(); } );
      }

      chain::database& db = appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >().db();
      ...
      {
         my->_enable_at_block = CreateCoin_HF20_BLOCK_NUM;
      }
#endif
