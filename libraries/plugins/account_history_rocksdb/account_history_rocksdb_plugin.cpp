...
#include <CreateCoin/plugins/account_history_rocksdb/account_history_rocksdb_plugin.hpp>
...
#define CreateCoin_NAMESPACE_PREFIX "CreateCoin::protocol::"
...
using CreateCoin::protocol::signed_block_header;
using CreateCoin::protocol::signed_transaction;

using CreateCoin::chain::operation_notification;
using CreateCoin::chain::transaction_id_type;

using CreateCoin::utilities::benchmark_dumper;
...
public:
   impl( account_history_rocksdb_plugin& self, const bpo::variables_map& options, const bfs::path& storagePath) :
      _self(self),
      _mainDb(appbase::app().get_plugin<CreateCoin::plugins::chain::chain_plugin>().db()),
      _storagePath(storagePath),
      _writeBuffer(_storage, _columnHandles)
      {
      collectOptions(options);

      _mainDb.add_pre_reindex_handler([&]( const CreateCoin::chain::reindex_notification& note ) -> void
         {
            on_pre_reindex( note );
         }, _self, 0);

      _mainDb.add_post_reindex_handler([&]( const CreateCoin::chain::reindex_notification& note ) -> void
         {
   ...
   void on_pre_reindex( const CreateCoin::chain::reindex_notification& note );
   void on_post_reindex( const CreateCoin::chain::reindex_notification& note );
...
void account_history_rocksdb_plugin::impl::collectOptions(const boost::program_options::variables_map& options)
{
   ...
   CreateCoin_LOAD_VALUE_SET(options, "account-history-rocksdb-track-account-range", _tracked_accounts, pairstring);
...
std::vector<account_name_type> account_history_rocksdb_plugin::impl::getImpactedAccounts(const operation& op) const
{
   ...
   CreateCoin::app::operation_get_impacted_accounts(op, impacted);
...
void account_history_rocksdb_plugin::impl::storeOpFilteringParameters(const std::vector<std::string>& opList,
   flat_set<std::string>* storage) const
   {
      for(const auto& arg : opList)
      {
         ...
         {
            if( op.empty() == false )
               storage->insert( CreateCoin_NAMESPACE_PREFIX + op );
         }
      }
   }
...
void account_history_rocksdb_plugin::impl::on_pre_reindex(const CreateCoin::chain::reindex_notification& note)
{
...
void account_history_rocksdb_plugin::impl::on_post_reindex(const CreateCoin::chain::reindex_notification& note)
{
...
FC_REFLECT( CreateCoin::plugins::account_history_rocksdb::account_history_info,
