...
#include <CreateCoin/plugins/witness/witness_plugin_objects.hpp>
...
#include <CreateCoin/utilities/plugin_utilities.hpp>
...
namespace detail {
   ...
   public:
      witness_plugin_impl( boost::asio::io_service& io ) :
         _timer(io),
         _chain_plugin( appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >() ),
      ...
      uint32_t _required_witness_participation  = 33 * CreateCoin_1_PERCENT;
      uint32_t _production_skip_flags           = chain::database::skip_nothing;

      std::map< CreateCoin::protocol::public_key_type, fc::ecc::private_key > _private_keys;
      std::set< CreateCoin::protocol::account_name_type >                     _witnesses;
   ...
   struct comment_options_extension_visitor
   {
      ...
      {
         CreateCoin_ASSERT( cpb.beneficiaries.size() <= 8,
            plugin_exception,
   ...
   void check_memo( const string& memo, const chain::account_object& account, const account_authority_object& auth )
   {
      ...
      {
         for( auto& key : keys )
            CreateCoin_ASSERT( key_weight_pair.first != key,  plugin_exception,
      ...
      for( auto& key : keys )
         CreateCoin_ASSERT( memo_key != key,  plugin_exception,
   ...
   struct operation_visitor
   {
      ...
      void operator()( const comment_operation& o )const
      {
         if( o.parent_author != CreateCoin_ROOT_POST_PARENT )
         {
            ...
            if( parent != nullptr )
            CreateCoin_ASSERT( parent->depth < CreateCoin_SOFT_MAX_COMMENT_DEPTH,
               plugin_exception,
               "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CreateCoin_SOFT_MAX_COMMENT_DEPTH) );
         }
      }
         ...
         case operation::tag< custom_binary_operation >::value:
            if( _db.is_producing() )
            {
               ...
               for( const account_name_type& account : impacted )
               {
                  ...
                  CreateCoin_ASSERT( !coo, plugin_exception,
   ...
   {
      if( fc::time_point::now() < fc::time_point(CreateCoin_GENESIS_TIME) )
      {
         wlog( "waiting until genesis time to produce block: ${t}", ("t",CreateCoin_GENESIS_TIME) );
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      {
         capture("pct", uint32_t(100*uint64_t(prate) / CreateCoin_1_PERCENT));
      ...
      appbase::app().get_plugin< CreateCoin::plugins::p2p::p2p_plugin >().broadcast_block( block );
...
{ try {
   ...
   CreateCoin_LOAD_VALUE_SET( options, "witness", my->_witnesses, CreateCoin::protocol::account_name_type )
   ...
   if( options.count("private-key") )
   {
      ...
      {
         fc::optional<fc::ecc::private_key> private_key = CreateCoin::utilities::wif_to_key(wif_key);
   ...
   {
      my->_required_witness_participation = CreateCoin_1_PERCENT * options.at( "required-participation" ).as< uint32_t >();
   }
...
{ try {
   ...
   chain::database& d = appbase::app().get_plugin< CreateCoin::plugins::chain::chain_plugin >().db();

   if( !my->_witnesses.empty() )
   {
      ilog( "Launching block production for ${n} witnesses.", ("n", my->_witnesses.size()) );
      appbase::app().get_plugin< CreateCoin::plugins::p2p::p2p_plugin >().set_block_production( true );
