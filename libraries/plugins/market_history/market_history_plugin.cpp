...
using CreateCoin::protocol::fill_order_operation;
      ...
      for( const auto& bucket : _tracked_buckets )
      {
         ...
         if( itr == bucket_idx.end() )
         {
            _db.create< bucket_object >( [&]( bucket_object& b )
            {
               ...
               b.CreateCoin.fill( ( op.open_pays.symbol == CreateCoin_SYMBOL ) ? op.open_pays.amount : op.current_pays.amount );
#ifdef CreateCoin_ENABLE_SMT
                  b.symbol = ( op.open_pays.symbol == CreateCoin_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
#endif
                  b.non_CreateCoin.fill( ( op.open_pays.symbol == CreateCoin_SYMBOL ) ? op.current_pays.amount : op.open_pays.amount );
            });
         }
         else
         {
            _db.modify( *itr, [&]( bucket_object& b )
            {
               ...
               b.symbol = ( op.open_pays.symbol == CreateCoin_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
#endif
               if( op.open_pays.symbol == CreateCoin_SYMBOL )
               {
                  b.CreateCoin.volume += op.open_pays.amount;
                  b.CreateCoin.close = op.open_pays.amount;

                  b.non_CreateCoin.volume += op.current_pays.amount;
                  b.non_CreateCoin.close = op.current_pays.amount;

                  if( b.high() < price( op.current_pays, op.open_pays ) )
                  {
                     b.CreateCoin.high = op.open_pays.amount;

                     b.non_CreateCoin.high = op.current_pays.amount;
                  }

                  if( b.low() > price( op.current_pays, op.open_pays ) )
                  {
                     b.CreateCoin.low = op.open_pays.amount;

                     b.non_CreateCoin.low = op.current_pays.amount;
                  }
               }
               else
               {
                  b.CreateCoin.volume += op.current_pays.amount;
                  b.CreateCoin.close = op.current_pays.amount;

                  b.non_CreateCoin.volume += op.open_pays.amount;
                  b.non_CreateCoin.close = op.open_pays.amount;

                  if( b.high() < price( op.open_pays, op.current_pays ) )
                  {
                     b.CreateCoin.high = op.current_pays.amount;

                     b.non_CreateCoin.high = op.open_pays.amount;
                  }

                  if( b.low() > price( op.open_pays, op.current_pays ) )
                  {
                     b.CreateCoin.low = op.current_pays.amount;

                     b.non_CreateCoin.low = op.open_pays.amount;
                  }
               }
            });
