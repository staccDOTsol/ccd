...
#include <CreateCoin/plugins/rc/rc_objects.hpp>
...
using namespace CreateCoin::chain::util;
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      // set a ridiculously high CreateCoin price ($1 / satoshi) to disable dust threshold
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = CreateCoin_ROOT_POST_PARENT;
      ...
      for( const auto& voter : voters )
      {
         ...
         vote.weight = CreateCoin_100_PERCENT;
      ...
      //auto reward_CreateCoin = db->get_dynamic_global_properties().total_reward_fund_CreateCoin;

      // generate a few blocks to seed the reward fund
      generate_blocks(10);
      //const auto& rf = db->get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      for( const auto& author : authors )
      {
         ...
         ilog( "${n} : ${CreateCoin} ${sbd}", ("n", author.name)("CreateCoin", a.reward_CreateCoin_balance)("sbd", a.reward_sbd_balance) );
      }
      for( const auto& voter : voters )
      {
         const account_object& a = db->get_account(voter.name);
         ilog( "${n} : ${CreateCoin} ${sbd}", ("n", voter.name)("CreateCoin", a.reward_CreateCoin_balance)("sbd", a.reward_sbd_balance) );
      }
      */
...
BOOST_AUTO_TEST_CASE( comment_payout_dust )
{
   try
   {
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.weight = 81 * CreateCoin_1_PERCENT;
      ...
      vote.voter = "bob";
      vote.author = "bob";
      vote.weight = 59 * CreateCoin_1_PERCENT;
      ...
      // If comments are paid out independent of order, then the last satoshi of CreateCoin cannot be divided among them
      const auto rf = db->get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      {
         const auto& post_rf = db->get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db->get< reward_fund_object, by_name >( CreateCoin_COMMENT_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db->get_comment( "alice", string( "test" ) ).net_rshares.value,
         db->get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME ).author_reward_curve,
         db->get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * CreateCoin_BLOCK_INTERVAL ) / CreateCoin_RECENT_RSHARES_DECAY_TIME_HF19.to_seconds();
         const auto& post_rf = db->get< reward_fund_object, by_name >( CreateCoin_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - CreateCoin_BLOCK_INTERVAL, true );

      auto reward_CreateCoin = db->get_dynamic_global_properties().total_reward_fund_CreateCoin + ASSET( "1.667 TESTS" );
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_CreateCoin.amount.value ) / total_rshares2 ).to_uint64(), CreateCoin_SYMBOL );
      auto bob_comment_discussion_rewards = asset( bob_comment_payout.amount / 4, CreateCoin_SYMBOL );
      bob_comment_payout -= bob_comment_discussion_rewards;
      auto bob_comment_sbd_reward = db->to_sbd( asset( bob_comment_payout.amount / 2, CreateCoin_SYMBOL ) );
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, CreateCoin_SYMBOL) ) * db->get_dynamic_global_properties().get_vesting_share_price();

      BOOST_TEST_MESSAGE( "Cause first payout" );

      generate_block();

      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_CreateCoin == reward_CreateCoin - bob_comment_payout );
      ...
      vote.voter = "dave";
      vote.author = "bob";
      vote.weight = CreateCoin_1_PERCENT;
      tx.operations.push_back( vote );
      sign( tx, dave_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db->head_block_time().sec_since_epoch() + CreateCoin_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - ( CreateCoin_BLOCK_INTERVAL / 2 ), true );
      ...
      auto reward_CreateCoin = db->get_dynamic_global_properties().total_reward_fund_CreateCoin + ASSET( "2.000 TESTS" );
      auto total_rshares2 = db->get_dynamic_global_properties().total_reward_shares2;
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_CreateCoin.amount.value ) / total_rshares2 ).to_uint64(), CreateCoin_SYMBOL );
      auto bob_comment_vote_rewards = asset( bob_comment_payout.amount / 2, CreateCoin_SYMBOL );
      bob_comment_payout -= bob_comment_vote_rewards;
      auto bob_comment_sbd_reward = asset( bob_comment_payout.amount / 2, CreateCoin_SYMBOL ) * exchange_rate;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, CreateCoin_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      auto unclaimed_payments = bob_comment_vote_rewards;
      auto alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), CreateCoin_SYMBOL );
      auto alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), CreateCoin_SYMBOL );
      auto bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), CreateCoin_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_CreateCoin.amount.value == reward_CreateCoin.amount.value - ( bob_comment_payout + bob_comment_vote_rewards - unclaimed_payments ).amount.value );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - ( CreateCoin_BLOCK_INTERVAL / 2 ), true );
      ...
      reward_CreateCoin = db->get_dynamic_global_properties().total_reward_fund_CreateCoin + ASSET( "2.000 TESTS" );
      ...
      u256 rf( reward_CreateCoin.amount.value );
      u256 trs2 = total_rshares2.hi;
      trs2 = ( trs2 << 64 ) + total_rshares2.lo;
      auto rs2 = rs*rs;

      auto alice_comment_payout = asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), CreateCoin_SYMBOL );
      auto alice_comment_vote_rewards = asset( alice_comment_payout.amount / 2, CreateCoin_SYMBOL );
      alice_comment_payout -= alice_comment_vote_rewards;
      auto alice_comment_sbd_reward = asset( alice_comment_payout.amount / 2, CreateCoin_SYMBOL ) * exchange_rate;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, CreateCoin_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      unclaimed_payments = alice_comment_vote_rewards;
      alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CreateCoin_SYMBOL );
      alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CreateCoin_SYMBOL );
      bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CreateCoin_SYMBOL );
      sam_vote_vesting = sam_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto dave_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "dave" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CreateCoin_SYMBOL );
      ...
      BOOST_REQUIRE( ( db->get_dynamic_global_properties().total_reward_fund_CreateCoin + alice_comment_payout + alice_comment_vote_rewards - unclaimed_payments ).amount.value == reward_CreateCoin.amount.value );
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      vote_op.weight = CreateCoin_100_PERCENT;
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = CreateCoin_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( CreateCoin_BLOCK_INTERVAL ), true );

      auto gpo = db->get_dynamic_global_properties();
      uint128_t reward_CreateCoin = gpo.total_reward_fund_CreateCoin.amount.value + ASSET( "2.000 TESTS" ).amount.value;
      ...
      auto alice_comment_reward = ( ( reward_CreateCoin * alice_comment.net_rshares.value * alice_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( alice_comment.net_rshares.value ) * ( alice_comment.net_rshares.value );
      reward_CreateCoin -= alice_comment_reward;
      auto alice_comment_vote_rewards = alice_comment_reward / 2;
      alice_comment_reward -= alice_comment_vote_rewards;

      auto alice_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), CreateCoin_SYMBOL );
      auto bob_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), CreateCoin_SYMBOL );
      reward_CreateCoin += alice_comment_vote_rewards - ( alice_vote_alice_reward + bob_vote_alice_reward ).amount.value;

      auto bob_comment_reward = ( ( reward_CreateCoin * bob_comment.net_rshares.value * bob_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( bob_comment.net_rshares.value ) * bob_comment.net_rshares.value;
      reward_CreateCoin -= bob_comment_reward;
      auto bob_comment_vote_rewards = bob_comment_reward / 2;
      bob_comment_reward -= bob_comment_vote_rewards;

      auto alice_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), CreateCoin_SYMBOL );
      auto bob_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), CreateCoin_SYMBOL );
      auto sam_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), CreateCoin_SYMBOL );
      reward_CreateCoin += bob_comment_vote_rewards - ( alice_vote_bob_reward + bob_vote_bob_reward + sam_vote_bob_reward ).amount.value;

      auto dave_comment_reward = ( ( reward_CreateCoin * dave_comment.net_rshares.value * dave_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( dave_comment.net_rshares.value ) * dave_comment.net_rshares.value;
      reward_CreateCoin -= dave_comment_reward;
      auto dave_comment_vote_rewards = dave_comment_reward / 2;
      dave_comment_reward -= dave_comment_vote_rewards;

      auto bob_vote_dave_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "dave", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * dave_comment_vote_rewards ) / dave_comment.total_vote_weight ), CreateCoin_SYMBOL );
      reward_CreateCoin += dave_comment_vote_rewards - bob_vote_dave_reward.amount.value;
      ...
      auto alice_comment_total_payout = db->to_sbd( asset( alice_pays_alice_sbd + alice_pays_alice_vest, CreateCoin_SYMBOL ) );
      alice_comment_total_payout += db->to_sbd( asset( bob_pays_alice_sbd + bob_pays_alice_vest, CreateCoin_SYMBOL ) );
      alice_comment_total_payout += db->to_sbd( asset( dave_pays_alice_sbd + dave_pays_alice_vest, CreateCoin_SYMBOL ) );
      auto bob_comment_total_payout = db->to_sbd( asset( bob_pays_bob_sbd + bob_pays_bob_vest, CreateCoin_SYMBOL ) );
      bob_comment_total_payout += db->to_sbd( asset( dave_pays_bob_sbd + dave_pays_bob_vest, CreateCoin_SYMBOL ) );
      auto sam_comment_total_payout = db->to_sbd( asset( dave_pays_sam_sbd + dave_pays_sam_vest, CreateCoin_SYMBOL ) );
      auto dave_comment_total_payout = db->to_sbd( asset( dave_pays_dave_sbd + dave_pays_dave_vest, CreateCoin_SYMBOL ) );
      ...
      auto alice_total_sbd = alice_starting_sbd + asset( alice_pays_alice_sbd + bob_pays_alice_sbd + dave_pays_alice_sbd, CreateCoin_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, CreateCoin_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto bob_total_sbd = bob_starting_sbd + asset( bob_pays_bob_sbd + dave_pays_bob_sbd, CreateCoin_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, CreateCoin_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto sam_total_sbd = sam_starting_sbd + asset( dave_pays_sam_sbd, CreateCoin_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, CreateCoin_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto dave_total_sbd = dave_starting_sbd + asset( dave_pays_dave_sbd, CreateCoin_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, CreateCoin_SYMBOL ) * gpo.get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db->head_block_time() + CreateCoin_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( CreateCoin_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < CreateCoin_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db->head_block_time() + CreateCoin_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == CreateCoin_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + CreateCoin_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         generate_blocks( db->head_block_time() + CreateCoin_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
         fill_op = get_last_operations( 1 )[0].get< fill_vesting_withdraw_operation >();
         gpo = db->get_dynamic_global_properties();

         BOOST_REQUIRE( db->get_account( "alice" ).next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + CreateCoin_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = CreateCoin_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = CreateCoin_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * CreateCoin_1_PERCENT * 20 ) / CreateCoin_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * CreateCoin_1_PERCENT * 50 ) / CreateCoin_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * CreateCoin_1_PERCENT * 30 ) / CreateCoin_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = CreateCoin_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CreateCoin_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      generate_blocks( db->get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * CreateCoin_1_PERCENT * 50 ) / CreateCoin_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / CreateCoin_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( CreateCoin_INIT_MINER_NAME, accounts[i], asset( 10000, CreateCoin_SYMBOL ) );
      ...
      ops[0].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset( 100000, CreateCoin_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset( 105000, CreateCoin_SYMBOL ) );
      ops[2].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  98000, CreateCoin_SYMBOL ) );
      ops[3].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  97000, CreateCoin_SYMBOL ) );
      ops[4].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  99000, CreateCoin_SYMBOL ) );
      ops[5].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset(  97500, CreateCoin_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 1000, SBD_SYMBOL ), asset( 102000, CreateCoin_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db->head_block_time() + CreateCoin_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( CreateCoin_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 1000, SBD_SYMBOL ), asset( 99000, CreateCoin_SYMBOL) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 1000, SBD_SYMBOL ), asset( 99000, CreateCoin_SYMBOL) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( ops[j].exchange_rate.base, asset( ops[j].exchange_rate.quote.amount + 10, CreateCoin_SYMBOL ) );
            txs[j].set_expiration( db->head_block_time() + CreateCoin_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + CreateCoin_CONVERSION_DELAY - fc::seconds( CreateCoin_BLOCK_INTERVAL / 2 ), true );
...
BOOST_AUTO_TEST_CASE( CreateCoin_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing CreateCoin Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( CreateCoin_MIN_CONTENT_REWARD, asset( ( CreateCoin_CONTENT_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) )
         + std::max( CreateCoin_MIN_CURATE_REWARD, asset( ( CreateCoin_CURATE_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) );
      auto witness_pay = std::max( CreateCoin_MIN_PRODUCER_REWARD, asset( ( CreateCoin_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_CreateCoin = asset( 0, CreateCoin_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_CreateCoin += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_CreateCoin );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_CreateCoin;
      new_rewards += gpo.total_reward_fund_CreateCoin;
      new_vesting_CreateCoin += gpo.total_vesting_fund_CreateCoin;
      ...
      BOOST_REQUIRE( gpo.total_reward_fund_CreateCoin.amount.value == new_rewards.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_fund_CreateCoin.amount.value == new_vesting_CreateCoin.amount.value );
      ...
      while( db->head_block_num() < CreateCoin_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( CreateCoin_MIN_CONTENT_REWARD, asset( ( CreateCoin_CONTENT_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) )
            + std::max( CreateCoin_MIN_CURATE_REWARD, asset( ( CreateCoin_CURATE_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) );
         witness_pay = std::max( CreateCoin_MIN_PRODUCER_REWARD, asset( ( CreateCoin_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) );
         new_vesting_CreateCoin = asset( 0, CreateCoin_SYMBOL );
         ...
         if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, CreateCoin_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_CreateCoin;
         new_rewards += gpo.total_reward_fund_CreateCoin;
         new_vesting_CreateCoin += gpo.total_vesting_fund_CreateCoin;
      ...
      while( db->head_block_num() < CreateCoin_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( CreateCoin_MIN_PRODUCER_REWARD, asset( ( CreateCoin_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_CreateCoin = asset( ( witness_pay + new_rewards ).amount * 9, CreateCoin_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < CreateCoin_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( CreateCoin_MIN_PRODUCER_REWARD, asset( ( CreateCoin_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CreateCoin_BLOCKS_PER_YEAR * 100 ), CreateCoin_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_CreateCoin = asset( ( witness_pay + new_rewards ).amount * 9, CreateCoin_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_CreateCoin;
         new_rewards += gpo.total_reward_fund_CreateCoin;
         new_vesting_CreateCoin += gpo.total_vesting_fund_CreateCoin;
      ...
      vesting_CreateCoin = gpo.total_vesting_fund_CreateCoin;
      reward_CreateCoin = gpo.total_reward_fund_CreateCoin;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_CreateCoin.amount.value,
         ( vesting_CreateCoin.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / CreateCoin_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / CreateCoin_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_CreateCoin.amount.value,
         reward_CreateCoin.amount.value + virtual_supply.amount.value / 10 / CreateCoin_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / CreateCoin_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db->get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / CreateCoin_BLOCKS_PER_YEAR ) * CreateCoin_1_PERCENT ) / CreateCoin_100_PERCENT, CreateCoin_SYMBOL ) * ( vesting_shares / vesting_CreateCoin ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( sbd_interest )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + fc::seconds( CreateCoin_SBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).sbd_balance.amount.value) == alice_sbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_sbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() ) / CreateCoin_SECONDS_PER_YEAR ) * gpo.sbd_interest_rate ) / CreateCoin_100_PERCENT ).to_uint64() );
      ...
      generate_blocks( db->head_block_time() + fc::seconds( CreateCoin_SBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      generate_blocks( db->head_block_time() + fc::seconds( ( CreateCoin_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      alice_CreateCoin_volume += ( asset( alice_sbd.amount / 20, SBD_SYMBOL ) * exchange_rate ).amount.value;
      alice_reward_last_update = db->head_block_time();
      bob_CreateCoin_volume -= ( asset( alice_sbd.amount / 20, SBD_SYMBOL ) * exchange_rate ).amount.value;
      ...
      op.owner = "sam";
      op.amount_to_sell = asset( ( alice_sbd.amount.value / 20 ), CreateCoin_SYMBOL );
      ...
      op.owner = "bob";
      op.orderid = 4;
      op.amount_to_sell = asset( ( alice_sbd.amount.value / 10 ) * 3 - alice_sbd.amount.value / 20, CreateCoin_SYMBOL );
      ...
      op.min_to_receive = asset( ( alice_sbd.amount.value / 10 ) * 3, CreateCoin_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( ( alice_sbd.amount.value / 10 ) * 3 - alice_sbd.amount.value / 20, CreateCoin_SYMBOL ).amount.value );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( alice_sbd.amount.value / 20, CreateCoin_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 5 );
      ...
      op.min_to_receive = asset( alice_sbd.amount.value / 20 * 2, CreateCoin_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( alice_sbd.amount.value / 20, CreateCoin_SYMBOL ).amount.value );
      ...
      alice_CreateCoin_volume += alice_sbd.amount.value / 20;
      alice_reward_last_update = db->head_block_time();
      sam_CreateCoin_volume -= alice_sbd.amount.value / 20;
      sam_reward_last_update = db->head_block_time();
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( 8 * ( alice_sbd.amount.value / 20 ), CreateCoin_SYMBOL );
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test1" ) ).reward_weight == CreateCoin_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db->head_block_time() + CreateCoin_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( CreateCoin_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test2" ) ).reward_weight == CreateCoin_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test3" ) ).reward_weight == CreateCoin_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test4" ) ).reward_weight == CreateCoin_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test5" ) ).reward_weight == CreateCoin_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = CreateCoin_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( sbd_stability )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().sbd_print_rate == CreateCoin_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db->get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * CreateCoin_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db->get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset sbd_balance = asset( ( gpo.virtual_supply.amount * ( gpo.sbd_stop_percent + 112 ) ) / CreateCoin_100_PERCENT, CreateCoin_SYMBOL ) * exchange_rate;
      ...
      auto comment_reward = ( gpo.total_reward_fund_CreateCoin.amount + 2000 ) - ( ( gpo.total_reward_fund_CreateCoin.amount + 2000 ) * 25 * CreateCoin_1_PERCENT ) / CreateCoin_100_PERCENT ;
      comment_reward /= 2;
      auto sbd_reward = ( comment_reward * gpo.sbd_print_rate ) / CreateCoin_100_PERCENT;
      auto alice_sbd = db->get_account( "alice" ).sbd_balance + db->get_account( "alice" ).reward_sbd_balance + asset( sbd_reward, CreateCoin_SYMBOL ) * exchange_rate;
      auto alice_CreateCoin = db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_CreateCoin_balance ;

      BOOST_TEST_MESSAGE( "Checking printing SBD has slowed" );
      BOOST_REQUIRE( db->get_dynamic_global_properties().sbd_print_rate < CreateCoin_100_PERCENT );

      BOOST_TEST_MESSAGE( "Pay out comment and check rewards are paid as CreateCoin" );
      db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );

      validate_database();

      BOOST_REQUIRE( db->get_account( "alice" ).sbd_balance + db->get_account( "alice" ).reward_sbd_balance == alice_sbd );
      BOOST_REQUIRE( db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_CreateCoin_balance > alice_CreateCoin );
      ...
      while( ( db->get_dynamic_global_properties().current_sbd_supply * exchange_rate ).amount >= ( db->get_dynamic_global_properties().virtual_supply.amount * db->get_dynamic_global_properties().sbd_start_percent ) / CreateCoin_100_PERCENT )
      {
         auto& gpo = db->get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( sbd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_sbd_supply, asset( ( CreateCoin_100_PERCENT ) * gpo.current_supply.amount, CreateCoin_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( CreateCoin_NULL_ACCOUNT ), [&]( account_object& a )
         {
            a.reward_CreateCoin_balance = ASSET( "1.000 TESTS" );
            a.reward_sbd_balance = ASSET( "1.000 TBD" );
            a.reward_vesting_balance = ASSET( "1.000000 VESTS" );
            a.reward_vesting_CreateCoin = ASSET( "1.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.pending_rewarded_vesting_CreateCoin += ASSET( "1.000 TESTS" );
         });
      });

      validate_database();

      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).sbd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_CreateCoin_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_vesting_CreateCoin == ASSET( "1.000 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).savings_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_sbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_CreateCoin_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CreateCoin_NULL_ACCOUNT ).reward_vesting_CreateCoin == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).sbd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( generate_account_subsidies )
{
   try
   {
      ...
      const witness_schedule_object& wso = db->get_witness_schedule_object();
      BOOST_CHECK_EQUAL( wso.account_subsidy_rd.resource_unit, CreateCoin_ACCOUNT_SUBSIDY_PRECISION );
      ...
      while( true )
      {
         ...
         if( gpo.available_account_subsidies >= 100 * CreateCoin_ACCOUNT_SUBSIDY_PRECISION )
            break;
         generate_block();
         pool = pool + 5123 - ((249617279 * pool) >> CreateCoin_RD_DECAY_DENOM_SHIFT);
      }
...
BOOST_AUTO_TEST_CASE( account_subsidy_witness_limits )
{
   try
   {
      ...
      size_t n = size_t( db->get< witness_object, by_name >( db->get_scheduled_witness( 1 ) ).available_witness_account_subsidies / CreateCoin_ACCOUNT_SUBSIDY_PRECISION );
