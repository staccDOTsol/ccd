...
extern uint32_t ( CreateCoin_TESTING_GENESIS_TIMESTAMP );

#define PUSH_TX \
   CreateCoin::chain::test::_push_transaction

#define PUSH_BLOCK \
   CreateCoin::chain::test::_push_block
...
/*#define CreateCoin_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CreateCoin_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CreateCoin_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define CreateCoin_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CreateCoin_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CreateCoin_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   CreateCoin_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   CreateCoin_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
#define ASSET( s ) \
   CreateCoin::plugins::condenser_api::legacy_asset::from_string( s ).to_asset()
...
{ \
   signed_transaction tx; \
   OP2TX(OP,tx,KEY) \
   CreateCoin_REQUIRE_THROW( db->push_transaction( tx, 0 ), EXCEPTION ); \
}
...
struct database_fixture {
   ...
   string debug_key = CreateCoin::utilities::key_to_wif( init_account_priv_key );
...
struct t_smt_database_fixture : public T
{
   ...
   //smt_setup_operation
   smt_generation_unit get_generation_unit ( const units& CreateCoin_unit = units(), const units& token_unit = units() );
   ...
   (
      ...
      const smt_cap_commitment& min_CreateCoin_units_commitment = smt_cap_commitment(),
      const smt_cap_commitment& hard_cap_CreateCoin_units_commitment = smt_cap_commitment(),
...
{
   private:
      CreateCoin::plugins::json_rpc::json_rpc_plugin* rpc_plugin;
