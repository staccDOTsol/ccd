...
function print_help_and_quit {
   echo "Usage: path_to_tested_CreateCoind path_to_reference_CreateCoind path_to_test_blockchain_directory"
   echo "       path_to_reference_blockchain_directory number_of_blocks_to_replay [number_of_jobs [--dont-copy-config]]"
   echo "Example: ~/work/CreateCoinit/CreateCoin/build/Release/programs/CreateCoind/CreateCoind ~/master/CreateCoinit/CreateCoin/build/Release/programs/CreateCoind/CreateCoind ~/CreateCoinit/CreateCoin/work1 ~/CreateCoinit/CreateCoin/work2 5000000"
...
function check_CreateCoind_path {
   echo Checking $1...
...
function run_test_group {
   ...
   echo Running ./$GROUP_TEST_SCRIPT $JOBS $TEST_CreateCoinD_PATH $REF_CreateCoinD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   ./$GROUP_TEST_SCRIPT $JOBS $TEST_CreateCoinD_PATH $REF_CreateCoinD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   EXIT_CODE=$?
   if [ $EXIT_CODE -ne 0 ]
   then
      EXIT_CODE=-1
...
check_CreateCoind_path $TEST_CreateCoinD_PATH
check_CreateCoind_path $REF_CreateCoinD_PATH
