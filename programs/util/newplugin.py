...
target_link_libraries( {plugin_provider}_{plugin_name} CreateCoinit_app CreateCoinit_chain CreateCoinit_protocol )
...
{{
   public:
      {plugin_name}_api( const CreateCoin::app::api_context& ctx );
...
class {plugin_name}_plugin : public CreateCoin::app::plugin
{{
   public:
      {plugin_name}_plugin( CreateCoin::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <CreateCoin/app/api_context.hpp>
#include <CreateCoin/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( CreateCoin::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      CreateCoin::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( CreateCoin::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const CreateCoin::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( CreateCoin::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( CreateCoin::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( CreateCoin::app::application* app )
   : plugin(app)
{{
...
CreateCoin_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (CreateCoin for plugins developed by CreateCoinit)")
