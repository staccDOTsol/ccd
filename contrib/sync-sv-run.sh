...
CreateCoinD_PID=`pgrep -f p2p-endpoint`
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! CreateCoindsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  SAVED_PID=`cat /tmp/CreateCoindpid`
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    gdb --batch --quiet -ex "thread apply all bt full" -ex "quit" /usr/local/CreateCoind-full/bin/CreateCoind /tmp/core.$SAVED_PID >> /tmp/stacktrace
    STACKTRACE=`cat /tmp/stacktrace`
    echo NOTIFYALERT! CreateCoindsync stacktrace from coredump:
  ...
  RUN_SV_PID=`pgrep -f /etc/service/CreateCoind`
  kill -9 $RUN_SV_PID
fi
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    CreateCoinD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $CreateCoinD_PID
    echo CreateCoindsync: waiting for CreateCoind to exit cleanly

    # wait 60 seconds for CreateCoind to exit, to be safe.
    let WAIT_TIME=0
    while ( kill -0 $CreateCoinD_PID ) && [[ WAIT_TIME -le 60 ]]; do
       sleep 1
       let WAIT_TIME++
    done

    echo CreateCoindsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo CreateCoindsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! CreateCoindsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo CreateCoindsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! CreateCoindsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo CreateCoindsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! CreateCoindsync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo CreateCoindsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifyCreateCoindsync: CreateCoindsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/CreateCoind`
    kill -9 $RUN_SV_PID
  fi
