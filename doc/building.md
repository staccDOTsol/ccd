...
Builds CreateCoind to be a consensus-only low memory node. Data and fields not
...
### BUILD_CreateCoin_TESTNET=[OFF/ON]

Builds CreateCoin for use in a private testnet. Also required for building unit tests.
...
For Ubuntu 16.04 users, after installing the right packages with `apt` CreateCoin
...
CreateCoin requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building CreateCoin on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which CreateCoin does).
...
Then the instructions are the same as for CreateCoin:
...
### Install CreateCoin dependencies:
...
CreateCoin. Until then, this will allow you to install boost 1.60.0.
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) CreateCoind

This will only build `CreateCoind`.
