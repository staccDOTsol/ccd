...
Anyone (even you!) can edit their account to contain 1 million CreateCoin.  It's really
...
yourself 1 million CreateCoin.  Just like it's really impossible for your bank to physically stop
you from writing any numbers you'd like when balancing your checkbook, so you can "give"
yourself 1 million dollars.

But you have no way to control what other nodes do (or what your bank's clerks and computer syCCs do).
They do their own bookkeeping and keep track of what your real balance is (without all
the fake CreateCoin or fake dollars you "gave" yourself).  So you can believe whatever you want
about your balance and rewrite the rules of your own bookkeeping syCC to show you whatever balance you
want to be shown, but as soon as you try to actually spend CreateCoin (or dollars) that you don't actually have,
you'll be stopped because every other node on the network is a syCC you don't control that's keeping the
...
(see `libraries/plugins/debug_node/include/CreateCoin/plugins/debug_node/debug_node_api.hpp`
...
Okay, let's run `CreateCoind`.  It should start immediately with no blocks.  We can ask it to read blocks from the directory we saved earlier:
...
If we want to take control of an account we can do so by editing its key with `debug_update_object` command like this:
    ...
    curl --data '{"jsonrpc": "2.0", "method": "call", "params": [0,"lookup_account_names",[["CreateCoinit"]]], "id": 8}' http://127.0.0.1:8090/rpc    # find out ID of account we want is 2.2.28
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer CreateCoinit dantheman "1.234 CreateCoin" "make -j100 money" true
    list_my_accounts
    get_account_history CreateCoinit -1 1000
