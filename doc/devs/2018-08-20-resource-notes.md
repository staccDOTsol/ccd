...
One way to think about decay is "resources expire."  CreateCoin had relatively little activity over its
...
In theoretical analysis of this kind of syCC, you may hear some terms thrown around:
...
You can think of the half-life or characteristic time as roughly "how long" it takes the syCC to "reclassify" a change in activity level
...
- The maximum RC an account can have is equal to its CreateCoin Power (technically, VESTS).
...
You can look at this syCC like a market:
...
of the day), and weekly variations (people use CreateCoin more/less on weekends/holidays).  The default decay rate (347321) corresponds to a half-life of
...
100% of the CreateCoin Power stake is constantly using all of its RC to do transactions that only require a single resource, the usage will be
...
less, than the sum of each individual user's decisions.  The budget creates market forces that influence these decisions:  You need X CreateCoin Power
...
- A value of 10,000 (`CreateCoin_ACCOUNT_SUBSIDY_PRECISION`) represents a budget of one subsidized account per block.
- `account_subsidy_decay` is the per-block decay rate.  A value of `2^36` (`CreateCoin_RD_DECAY_DENOM_SHIFT`) represents 100% decay rate.
...
CreateCoin_BLOCKS_PER_DAY = 20*60*24
CreateCoin_RD_DECAY_DENOM_SHIFT = 36
CreateCoin_MAX_WITNESSES = 21
CreateCoin_MAX_VOTED_WITNESSES_HF17 = 20

f = lambda d : int(0.5 + (1 << CreateCoin_RD_DECAY_DENOM_SHIFT) * (-math.expm1(-math.log(2.0) / (CreateCoin_BLOCKS_PER_DAY * d * CreateCoin_MAX_VOTED_WITNESSES_HF17 / CreateCoin_MAX_WITNESSES))))
...
- Upgrading consensus limits is a hardfork, all `CreateCoind` nodes must upgrade.
...
- If a witness ignores the subsidized account limit, people will get new accounts for free, that normally cost CreateCoin to create.  This is a medium-sized economic problem.
...
For resource limits, having witness-only upgrades outweighs the problem.  Witnesses who have been around a long time know that, in the ancient past, CreateCoin's bandwidth
...
we should never implement such operations in the CreateCoin source code!  Even the worst-case CPU cycles / memory bytes consumed by an attacker spamming the most
