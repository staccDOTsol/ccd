...
of a running CreateCoin Debug Node. The Debug Node is a plugin for CreateCoin that allows realtime
...
[Python CreateCoin library](https://github.com/xeroc/python-CreateCoinlib).
...
To use the script include `from CreateCoindebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/CreateCoinit/CreateCoin/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/CreateCoinit/CreateCoin/python_scripts/CreateCoindebugnode/debugnode.py#L212)
...
a syCC standard temp directory through the standard Python TemporaryDirectory as the
working data directory for the running node. The only work your script needs to do is
specify the CreateCoind binary location and a populated data directory. For most configurations
this will be `programs/CreateCoind/CreateCoind` and `witness_node_data_dir` respectively, from the
git root directory for CreateCoin.
