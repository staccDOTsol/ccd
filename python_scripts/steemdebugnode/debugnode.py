...
from CreateCoinapi.CreateCoinnoderpc import CreateCoinNodeRPC

class DebugNode( object ):
   """ Wraps the CreateCoind debug node plugin for easier automated testing of the CreateCoin Network"""

   def __init__( self, CreateCoind, data_dir, args='', plugins=[], apis=[], CreateCoind_out=None, CreateCoind_err=None ):
      """ Creates a CreateCoind debug node.
      ...
      For all other requests, the python-CreateCoin library should be used.

      args:
         CreateCoind -- The string path to the location of the CreateCoind binary
         data_dir -- The string path to an existing CreateCoind data directory which will be used to pull blocks from.
         args -- Other string args to pass to CreateCoind.
         ...
         CreateCoind_stdout -- A stream for CreateCoind's stdout. Default is to pipe to /dev/null
         CreateCoind_stderr -- A stream for CreateCoind's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._CreateCoind_bin = None
      self._CreateCoind_lock = None
      self._CreateCoind_process = None
      self._temp_data_dir = None

      self._CreateCoind_bin = Path( CreateCoind )
      if( not self._CreateCoind_bin.exists() ):
         raise ValueError( 'CreateCoind does not exist' )
      if( not self._CreateCoind_bin.is_file() ):
         raise ValueError( 'CreateCoind is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed CreateCoin data directory' )
      ...
      if( CreateCoind_out != None ):
         self.CreateCoind_out = CreateCoind_out
      else:
         self.CreateCoind_out = self._FNULL

      if( CreateCoind_err != None ):
         self.CreateCoind_err = CreateCoind_err
      else:
         self.CreateCoind_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._CreateCoind_lock = Lock()


   def __enter__( self ):
      self._CreateCoind_lock.acquire()
      ...
      CreateCoind = [ str( self._CreateCoind_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      CreateCoind.extend( self._args )

      self._CreateCoind_process = Popen( CreateCoind, stdout=self.CreateCoind_out, stderr=self.CreateCoind_err )
      self._CreateCoind_process.poll()
      sleep( 5 )
      if( not self._CreateCoind_process.returncode ):
         self._rpc = CreateCoinNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "CreateCoind did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._CreateCoind_process != None ):
         self._CreateCoind_process.poll()

         if( not self._CreateCoind_process.returncode ):
            self._CreateCoind_process.send_signal( SIGINT )

            sleep( 7 )
            self._CreateCoind_process.poll()

            if( not self._CreateCoind_process.returncode ):
               self._CreateCoind_process.send_signal( SIGTERM )

               sleep( 5 )
               self._CreateCoind_process.poll()

               if( self._CreateCoind_process.returncode ):
                  loggin.error( 'CreateCoind did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._CreateCoind_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._CreateCoind_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key CreateCoin debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is CreateCoin_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both CreateCoind and the data directory,
      ...
      if( os.name != "posix" ):
         print( "This script only works on POSIX syCCs" )
         return
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--CreateCoind', '-s', type=str, required=True, help='The location of a CreateCoind binary to run the debug node' )
      ...
      CreateCoind = Path( args.CreateCoind )
      if( not CreateCoind.exists() ):
         print( 'Error: CreateCoind does not exist.' )
         return

      CreateCoind = CreateCoind.resolve()
      if( not CreateCoind.is_file() ):
         print( 'Error: CreateCoind is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed CreateCoind data directory' )
      ...
      debug_node = DebugNode( str( CreateCoind ), str( data_dir ), CreateCoind_err=sys.stderr )
